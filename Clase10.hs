...
-- Ejercicios -----------
--1--
tieneSoluc :: (Int, Int, Int) -> Bool
tieneSoluc (a,b,c) = mod b (mcd a c) == 0

cadaEcTieneSoluc :: [(Int, Int, Int)] -> Bool
cadaEcTieneSoluc [] = True
cadaEcTieneSoluc (x:xs) = (tieneSoluc x) && cadaEcTieneSoluc xs

--2--
tieneSolucionSimplif :: [(Int, Int)] -> Bool
tieneSolucionSimplif xs = tieneSolucionSimplifAux xs (todosLosPrimosMalos xs)

tieneSolucionSimplifAux :: [(Int, Int)] -> [Int] -> Bool
tieneSolucionSimplifAux eqs [] = True
tieneSolucionSimplifAux eqs (x:xs) = checkConguenciasMismoMod (fst listaSep) && tieneSolucionSimplifAux eqs xs
                              where listaSep = desdoblarSistemaEnFcionPrimo eqs x

checkConguenciasMismoMod:: [(Int,Int)] -> Bool
checkConguenciasMismoMod [(a,b)] = True
checkConguenciasMismoMod ((a,b):(c,d):xs) = ((mod a minMod) == (mod c minMod)) && checkConguenciasMismoMod ((c,d):xs)
                              where minMod = min b d

--3-- 
tieneSolucion :: [(Int, Int, Int)] -> Bool
tieneSolucion xs = cadaEcTieneSoluc xs && (tieneSolucionSimplif (sistemaSimplifEquiv xs))

--4--
dirichlet :: Int -> Int -> Int
dirichlet r m| Clase_adicional.esPrimo r = r
             | otherwise = dirichlet (r+m) m 
